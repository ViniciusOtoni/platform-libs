...
> **Correção (achada na revisão final de branch):** `validate_endpoint_name` (Task 3,
> `naming.py`) era definida e testada mas nunca chamada em `_online_endpoint` — mesmo
> gap já achado e corrigido em `validate_model_name` no `training-platform`. `domain`/
> `model_name` são texto livre do `ServingConfig`, fora do controle deste framework;
> sem a validação, um valor inválido só apareceria tarde, como um erro opaco do DAB ou
> da API de Model Serving. Corrigido chamando `validate_endpoint_name(endpoint_name)`
> logo após `derive_endpoint_name(...)` em `_online_endpoint`.
